...
ngx_http_output_header_filter_pt ngx_http_hf_next_header_filter;
...
ngx_int_t ps_etag_header_filter(ngx_http_request_t* r) {
  ...
  for (i = 0 ; /* void */; i++) {
    ...
    if (STR_CASE_EQ_LITERAL(header[i].key, "@psol-etag")) {
  ...
  return ngx_http_hf_next_header_filter(r);
}
...
ngx_int_t ps_etag_filter_init(ngx_conf_t* cf) {
  ...
  if (cfg_m->driver_factory != NULL) {
    ngx_http_hf_next_header_filter = ngx_http_top_header_filter;
